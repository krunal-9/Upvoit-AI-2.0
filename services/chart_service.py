...
from services.langgraph_sql_agent_charts import run_chart_generation
from utils.sql import format_sql
from utils.mongo_utils import make_mongo_safe, serialize_mongo_value
from db.mongo import dashboard_logs


async def process_chart(body, user_info):
    ...
    try:
        ...
        # 🔥 Call existing async engine
        result = await run_chart_generation(
            query=query,
        ...
        insert = dashboard_logs.insert_one(make_mongo_safe(log_doc))
...
def list_chart_logs(
    ...
    # ---- Fetch from Mongo ----
    total_records = dashboard_logs.count_documents(filter_query)

    records = list(
        dashboard_logs.find(filter_query, projection)
