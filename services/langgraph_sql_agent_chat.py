...
from typing import TypedDict, Annotated, Sequence, List, Dict, Any, Optional, Tuple, Literal
...
6. AMBIGUITY HANDLING (NEW):
   - If the user's request is ambiguous (e.g., "Show me the project" without specifying which one, or "Show me data" without context), you MUST use the `ask_user` tool to request clarification.
   - **CRITICAL: BUSINESS LANGUAGE ONLY.**
     - When asking the user, **NEVER** mention table names (e.g., 'CompanyUsers'), column names (e.g., 'CreatedDate'), or raw IDs/Enums.
     - **CORRECT:** "Do you want to filter by the date they joined the company or the date their user account was created?"
     - **INCORRECT:** "Should I filter by CompanyUsers.CreatedDate or Users.CreatedDate?"
     - **CORRECT:** "Do you mean technicians, managers, or all staff?"
     - **INCORRECT:** "Should I filter for PermissionLevel 3 or 4?"
   - Do not guess constraints if they are significant. Ask the user politely in plain English.

7. SECURITY SHIELD (HIGHEST PRIORITY):
...
5. Current UTC Time is: {current_time}. You MUST calculate the specific start and end dates based on this time and the user's request.
   - DO NOT use dynamic SQL functions like GETDATE(), GETUTCDATE(), or DATEADD() to determine the range.
   - INSTEAD, calculate the exact start and end dates (YYYY-MM-DD) mentally and use them as STRING LITERALS in the query.
   - Example: WHERE CreatedDate >= '2025-11-01' AND CreatedDate < '2025-12-01'
   - Use calendar weeks/months/quarters unless specified otherwise.
...
def ask_user(question: str):
    """Ask the user a clarifying question when the request is ambiguous or missing information."""
    return question
...
def table_selector(state: AgentState) -> AgentState:
    try:
        ...
        chat_history = []
        for msg in state.get("messages", [])[:-1]: # Exclude current user query if it's last (it might be in messages)
            if isinstance(msg, HumanMessage):
                chat_history.append(f"User: {msg.content}")
            elif isinstance(msg, AIMessage):
                chat_history.append(f"Assistant: {msg.content}")
        
        history_text = "\n".join(chat_history[-5:]) # Last 5 turns
        
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are a database expert. Your task is to select the most relevant tables for the user's query, considering the conversation history.
...
def query_generator(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # --- Context Extraction ---
        # Find the last successful SQL query from conversation history to help with context
        previous_sql = "None"
        chat_history = []
        reversed_msgs = state.get("messages", [])[::-1]
        
        # Simple heuristic: Look for the last AIMessage that contained "Generated SQL"
        for msg in reversed_msgs:
            if isinstance(msg, AIMessage) and "Generated SQL" in str(msg.content):
                if ":" in str(msg.content):
                    previous_sql = str(msg.content).split(":", 1)[1].strip()
                break
        ...
        schema_text = json.dumps(table_schemas, indent=2)

        table_list_str = ", ".join(table_schemas.keys())

        # Construct the detailed system prompt
        prompt = get_sql_generation_prompt()
        
        # Bind the clarification tool
        llm_with_tools = smart_llm.bind_tools([ask_user])
        chain = prompt | llm_with_tools

        response = chain.invoke(
            {
                "user_query": state["user_query"],
                "selected_tables": table_list_str,
                "schema": schema_text,
                ...
                "messages": state["messages"][-5:], # Pass recent messages for tool context
            }
        )

        # Check for tool_calls
        if response.tool_calls:
            log_agent_step("QueryGenerator", "Decided to ask user for clarification", {"tool_calls": response.tool_calls})
            return {
                **state,
                "messages": state["messages"] + [response], # Add the AIMessage with tool_calls
                "last_scratchpad": None,
                "iteration_count": iteration + 1, # Increment iteration even if asking for clarification
            }

        # If no tool call, assume it's SQL generation scratchpad+sql
        ...
        generated_query, scratchpad_text = extract_sql(llm_output)
...
def clarification_node(state: AgentState):
    ...
    With interrupt_after, we just let this node finish, then the graph pauses.
    We don't need to do anything here except ensure the tool call is recognized.
    """
    # We log it
    last_message = state["messages"][-1]
    if isinstance(last_message, AIMessage) and last_message.tool_calls:
        question = last_message.tool_calls[0]["args"].get("question")
        log_agent_step("ClarificationNode", "Pausing for user input", {"question": question})
    
    # Return nothing, just let the graph edge handle transition to query_generator
    return {}
...
# Conditional edge from query_generator
def route_query_gen(state: AgentState):
    last_message = state["messages"][-1]
    if isinstance(last_message, AIMessage) and last_message.tool_calls:
        return "clarification_node"
    return "query_executor"

workflow.add_conditional_edges("query_generator", route_query_gen)
workflow.add_edge("clarification_node", "query_generator") # Explicit edge instead of Command
...
async def run_conversational_query(
    ...
    # Classify query intent if it's a new conversation (no previous messages) or forceful
    # Actually, for resuming, we might skip this. But typically prompt comes in as query.
    ...
    intent = "data_query"
    # Only classify if we are NOT potentially resuming a paused graph
    # If state exists, we might be resuming or starting new.
    # If next is empty, we are starting new.
    if is_resuming and not state_snapshot.next:
        is_resuming = False
        
    if not is_resuming:
        intent = classify_query_intent(sanitized_query)
        log_agent_step("System", f"Query classified as: {intent}", {"run_id": thread_id})

        # Handle intents
        if intent == "malicious_query":
             # ... existing malicious handling ...
             return {
                "summary": "⚠️ Nice try! I cannot fulfill this request as it violates my security policies.",
                "results": [],
                "sql_query": "",
                "error": None,
                "selected_tables": [],
                "iteration_count": 0,
                "is_empty_result": False,
                "natural_response": "⚠️ Security Alert: Query blocked.",
                "scratchpad": None,
                "thread_id": thread_id,
            }
        elif intent == "general_query":
            res = handle_general_query(sanitized_query)
    ...
    inputs = None
    if is_resuming:
        logger.info(f"DEBUG: Resuming with input: {query}")
        # We need to find the tool call to reply to
        ...
        if isinstance(last_msg, AIMessage) and last_msg.tool_calls:
            ...
            logger.info("DEBUG: Updating state with ToolMessage (as_node=clarification_node)")
        ...
        # Start new
        logger.info("DEBUG: Starting NEW conversation.")
        inputs = {
            "run_id": thread_id,
            "messages": [HumanMessage(content=sanitized_query)],
            "user_query": sanitized_query,
    ...
    # Check if we paused (interrupt_after)
    # Be careful: app.invoke returns the state at the end of the run (or where it stopped)
    ...
    if snapshot.next:
        ...
        # Check if it stopped at clarification_node (next should be query_generator? No, next is the NEXT node)
        # If we interrupted AFTER clarification_node, next is query_generator (via Command)
        
        # We check if the last message was a tool call asking for clarification
        question = "Can you clarify?"
        if isinstance(last_msg, AIMessage) and last_msg.tool_calls:
             question = last_msg.tool_calls[0]['args'].get('question', question)
             ...
             return {
                 ...
                 "sql_query": "",
                 "error": None,
                 "natural_response": question,
                 "is_clarification": True, # Signal to UI
                 "thread_id": thread_id,
                 "scratchpad": None
             }
    ...
    if final_state.get("execution_result") and not final_state.get(
        ...
        if "dangerous operations" in str(final_state["error"]):
            summary = "I apologize, but for security reasons, I am a READ-ONLY assistant. I cannot modify, delete, or update data in the database."
        else:
            summary = f"Something went wrong while processing your request. Error: {final_state['error']}"
    else:
        # Generate a friendly "No results" message
        try:
            no_result_prompt = f"""The user asked a query but the SQL execution returned 0 rows.
            Explain this politely to the user. Mention what was searched for based on the query.
            
            User Query: {query}
            Executed SQL: {final_state.get("generated_query", "")}
            
            Your response (1 sentence, friendly):"""
            
            chain = ChatPromptTemplate.from_messages([
                ("system", "You are a friendly assistant."),
                ("human", no_result_prompt)
            ]) | llm | StrOutputParser()
            summary = chain.invoke({})
        except Exception:
